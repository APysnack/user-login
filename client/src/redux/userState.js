...
export const registerUser = createAsyncThunk(
    'user/registerUser', 
    (payload) => {
        return axios.post('http://localhost:3000/users', payload).then((res) => assembleResponse(res)).catch((error) => error)
    }
);
...
export const loginUser = createAsyncThunk(
    'user/loginUser', 
    (payload) => {
        return axios.post('http://localhost:3000/users/sign_in', payload).then((res) => assembleResponse(res)).catch((error) => error)
    }
);
...
export const loginWithToken = createAsyncThunk(
    'user/loginWithToken', 
    (config) => {
        return axios.get('http://localhost:3000/member-data', config).then((res) => assembleResponse(res)).catch((error) => error)
    }
);
...
export const logoutUser = createAsyncThunk(
    'user/logoutUser', 
    (config) => {
        return axios.delete('http://localhost:3000/users/sign_out', config).then((res) => assembleResponse(res)).catch((error) => error.message)
    }
);
